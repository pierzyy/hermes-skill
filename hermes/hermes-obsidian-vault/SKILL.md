...
```bash
# NAS 端（已配置）
# - post-commit hook: 自动 push 到 GitHub + bare repo
# - cron: */15 * * * * /bin/bash /opt/data/scripts/sync_skills.sh

# 云机端（通过 setup-cloud-skills.sh 初始化）
# - clone GitHub repo
# - post-commit hook: 自动 push 到 GitHub
# - cron: */15 * * * * /bin/bash /opt/data/scripts/sync_skills.sh
```
